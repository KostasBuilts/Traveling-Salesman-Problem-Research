...
int try_3opt_move(TSPInstance *instance, int *tour, int i, int j, int k) 
{
    ...
    double current = instance->dist_matrix[a][b] + instance->dist_matrix[c][d] + instance->dist_matrix[e][f];
    ...
    double new1 = instance->dist_matrix[a][c] + instance->dist_matrix[b][e] + instance->dist_matrix[d][f];
    ...
    double new2 = instance->dist_matrix[a][d] + instance->dist_matrix[e][b] + instance->dist_matrix[c][f];
    ...
    double new3 = instance->dist_matrix[a][e] + instance->dist_matrix[d][b] + instance->dist_matrix[c][f];
...
double enhanced_qmc_step(TSPInstance *instance, Replica *rep, double beta, double gamma, int step, int total_steps) 
{
    int n = instance->n;
...
void adaptive_parallel_tempering(Replica **replicas, double *temperatures, int num_replicas, double *acceptance_rates) 
{
...
void adjust_temperatures(double *temperatures, int num_replicas, double *acceptance_rates, int steps) 
{
        ...
        for (int step = 0; step < total_steps; step++) 
        {
            ...
            for (int w = 0; w < num_walkers; w++) 
            {
                for (int r = 0; r < P; r++) 
                {
                    ...
                    enhanced_qmc_step(instance, walkers[w][r], beta, gamma, step, total_steps);
